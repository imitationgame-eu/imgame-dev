...
function getData() {
  ...
  $.ajax({
    type: 'GET',
    url: '/webServices/step4/getStep4MonitorDetail.php',
    data: paramSet,
    dataType: 'json',
...
var transcriptsViewModel = function(data, target) {
  ...
  this.ratedInfo = ko.computed(function() {
    return viewModel.turnbyturn() == 1 ? (_this.rated() == 1 ? " (finished)" : " (not yet marked)") : "";
  });
  this.showRatings = ko.computed(function() {
    return viewModel.turnbyturn() == 0;
  });
  this.questionNo = ko.computed(function(){
    if (viewModel.turnbyturn() == 1) {
      var finalRating = '';
      if (_this.isFinalRating() == 1) { finalRating = '_final';}
      return '.q_'+_this.qNo() + finalRating;
    }
    else {
      return '';
    }
...
var turnsViewModel = function(data, target) {
  var _this = this;
  this.showDetail = ko.computed(function() {
    return viewModel.turnbyturn() === 0;
  });
  this.ratingDetail = ko.computed(function() {
    return viewModel.turnbyturn() === 1 ? (_this.rated() == 1 ? "marked" : "not yet marked") : "";
  });
  ko.mapping.fromJS(data, target, this);
}
// --------------------------------------------------- Mappings ------ //
