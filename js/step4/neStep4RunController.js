...
var actualJNo, s3respNo, respNo, qNo; // used when S4 is doing single turns
var urlParams = {};
var srcName;
var url;
var index;
...
function sendAction(messageType, content) {
  ...
  $.ajax({
    type: 'GET',
    url: '/webServices/step4/neStep4RunController.php',
    data: paramSet,
    dataType: 'text',
...
function processData(data) {
  ...
  switch (messageType) {
    ...
    case "step4Transcript":
      ...
      shuffleHalf = parseInt(xmlDoc.getElementsByTagName("shuffleHalf")[0].firstChild.nodeValue);
      actualJNo = parseInt(xmlDoc.getElementsByTagName("actualJNo")[0].firstChild.nodeValue);
      respNo1 = parseInt(xmlDoc.getElementsByTagName("respNo1")[0].firstChild.nodeValue);
      respNo2 = parseInt(xmlDoc.getElementsByTagName("respNo2")[0].firstChild.nodeValue);
      qNo = parseInt(xmlDoc.getElementsByTagName("qNo")[0].firstChild.nodeValue);
      s3respNo1 = parseInt(xmlDoc.getElementsByTagName("s3respNo1")[0].firstChild.nodeValue);
      s3respNo2 = parseInt(xmlDoc.getElementsByTagName("s3respNo2")[0].firstChild.nodeValue);
    ...
    case 's4complete' :
      ...
      if (usePost == 'usePost') {
        // do postSurvey
        var url="/sf_" + exptId + '_11_' + jType + '_' + s4jNo;
        var paramItems = {};
...
$(window).load(function() {
  ...
  surveyFlag = $('#hiddensurveyFlag').text();
  if (s4jNo == "-1") {
    // all s4 judges have started - no more can be allocated
    $('#tabOne').removeClass('empty').addClass('exptClosed');
    setTabOne();
  }
  else {
    var contentArray = {};
    contentArray[0] = surveyFlag;
    //contentArray[1] = 'noResponse';
    content = contentArray;
...
//  if ($radios.filter('[value=right_judgement]').attr('checked')) {choice='1';}            
  ...
  if (useS4QCategoryControl == 1) {
    if (categoryIntervalId == '') { validated=false; }    
  }
  if (useS4AlignmentControl == 1) {
...
function setRatingControls() {
  ...
  $('#s4nextB').val(step4_nextBLabel);
    $('#s4nextB').click( function(e) {
      s4IntentionText = $('#iIntention').val();
      reason = $('#jReason').val();
      if (pretenderRight == 0) {
        pAlignment = respondent1IntervalId;
        npAlignment = respondent2IntervalId;
      }
      else {
        pAlignment = respondent2IntervalId;
        npAlignment = respondent1IntervalId;        
      }
      var contentArray = {};
      ...
      contentArray[13] = categoryIntervalId;
