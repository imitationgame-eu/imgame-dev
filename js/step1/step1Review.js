...
var questionsViewModel = function (data, target, parent) {
  ...
  this.editPRID = ko.computed(function() {
    return 'editPR_' + _this.jqNo();
  });
  this.editingPR = ko.observable(false);
  this.stopEditingPR = function() {
    _this.editingPR(false);
  }
  this.editPR = function () {
    _this.editingPR(true);
    $(_this.editPRID()).focus();
  };

  this.editNPRID = ko.computed(function() {
    return 'editNPR_' + _this.jqNo();
  });
  this.editingNPR = ko.observable(false);
  this.editNPR = function () {
    _this.editingNPR(true);
    $(_this.editNPRID()).focus();
  };
  this.stopEditingNPR = function() {
    _this.editingNPR(false);
  }
...
function saveData() {
  ...
  var postRequest = $.ajax({
     url: "/webServices/step1/storeStep1ReviewedData.php",
     type: "POST",
