...
//    contentArray[13] = isFullRating;
    ...
    if ($isFullRating == 0) {
      ...
      $update = sprintf("UPDATE wt_TBTStep4datasets SET rated=1 WHERE s4jNo='%s' AND exptId='%s' AND jNo='%s' AND qNo='%s' AND isFinalRating='0'",
        $s4jNo, $exptId, $actualJNo, $qNo);
    }
    else {
      // update status of finalRating row to move onto next Q
      $update = sprintf("UPDATE wt_TBTStep4datasets SET rated=1 WHERE s4jNo='%s' AND exptId='%s' AND jNo='%s' AND isFinalRating='1'",
  ...
  private function getTurn($s4jNo) {
    ...
    $anyExptIdqry = sprintf("SELECT * FROM wt_TBTStep4datasets WHERE exptId='%s' AND s4jNo='%s' AND rated=0", $this->exptId, $s4jNo);
    //echo $anyExptIdqry.PHP_EOL;
    ...
    if ($igrtSqli->affected_rows !=0 ) {
      ...
      $getMinIGqry = sprintf("SELECT MIN(jNo) AS jNo FROM wt_TBTStep4datasets WHERE s4jNo='%s' AND exptId='%s' AND rated=0",
        $s4jNo, $this->exptId);
      //echo $getMinIGqry.PHP_EOL;
      $minIGResult = $igrtSqli->query($getMinIGqry);
      $minIGRow = $minIGResult->fetch_object();
      $jNo = $minIGRow->jNo;
      // get next qNo
      $getMinQNoqry = sprintf("SELECT MIN(qNo) as qNo,isFinalRating,respNo FROM wt_TBTStep4datasets WHERE s4jNo='%s' AND exptId='%s' AND jNo='%s' AND rated=0",
        $s4jNo, $this->exptId, $jNo);
      //echo $getMinQNoqry.PHP_EOL;
      $minQNoResult = $igrtSqli->query($getMinQNoqry);
      $minQNoRow = $minQNoResult->fetch_object();
      if ($minQNoRow->isFinalRating == "1") {
      	$respNo = $minQNoRow->respNo;
        $turnValues = $this->getAllTurnValues($this->exptId, $jNo, $respNo);
        ...
        $qNo = $minQNoRow->qNo;
        $respNo = $minQNoRow->respNo;
        $turnValues = $this->getTurnValues($this->exptId, $jNo, $qNo, $respNo);
        $pretenderRight = mt_rand(0,1);
  ...
  private function getTurnValues($exptId, $jNo, $qNo, $respNo) {
    global $igrtSqli;    
    $qList =[];
    $pList = [];
    $npList = [];
    $qry = sprintf("SELECT * FROM md_dataStep1reviewed "
      . "WHERE exptId='%s' AND dayNo=1 AND sessionNo=1 AND jType=0 AND jNo='%s' AND qNo='%s'",
      $exptId, $jNo, $qNo);
    $result = $igrtSqli->query($qry);
    $row = $result->fetch_object();
    $pqry = sprintf("SELECT * FROM md_dataStep2reviewed "
      . "WHERE exptId='%s' AND jType=0 AND canUse=1 AND actualJNo='%s' AND qNo='%s' AND respNo='%s'",
      $exptId, ($jNo+1), $qNo, $respNo);
    //echo $pqry;
...
//    }
  ...
  private function getAllTurnValues($exptId, $jNo, $respNo) {
    global $igrtSqli;    
    $qList =[];
    $pList = [];
    $npList = [];
    $qry = sprintf("SELECT * FROM md_dataStep1reviewed "
      . "WHERE exptId='%s' AND dayNo=1 AND sessionNo=1 AND jType=0 AND canUse=1 AND jNo='%s' ORDER BY qNo ASC",
      $exptId, $jNo);
    $result = $igrtSqli->query($qry);
    while ($row = $result->fetch_object()) {
      if ($row->q != 'FINAL') {
		...
		$pqry = sprintf("SELECT * FROM md_dataStep2reviewed "
			. "WHERE exptId='%s' AND jType=0 AND canUse=1 AND actualJNo='%s' AND respNo='%s' ORDER BY qNo ASC",
			$exptId, ($jNo+1), $respNo);
		//echo $pqry;
		$presult = $igrtSqli->query($pqry);
		for ($i=0; $i<count($qList); $i++) {
			$prow = $presult->fetch_object();
			array_push($pList, $prow->reply);
		}
    return [
      'q'=> $qList,
      'pr'=> $pList,
