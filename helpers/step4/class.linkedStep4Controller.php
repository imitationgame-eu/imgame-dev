...
class step4Controller {
  ...
  public function leGetStep4CurrentStatus($s4jNo) {
    $transcriptArray = $this->getWholeTranscript($s4jNo);
    return $transcriptArray;
  }
  
  public function storeTranscript($s4jNo, $content) {
    $this->storeWholeTranscript($s4jNo, $content);
  }
...
//      contentArray[14] = s3respNo; 
  ...
  private function storeWholeTranscript($s4jNo, $content) {
    global $igrtSqli;
    //echo print_r($content, true);
    $pretenderRight = $content[5];
    $choice = $content[2];
    if ($pretenderRight == 0) {
      // pretender is left - we are looking for the NP in linked step4
    ...
    $exptId = $content[8];
    $insert = sprintf("INSERT INTO dataLinkedSTEP4 "
      . "(exptId, jType, s4jNo, igNo, correct, choice, pretenderRight, confidence, reason) "
      ...
      $pretenderRight,
      $content[3],
      $igrtSqli->real_escape_string($reason)
      );
    $igrtSqli->query($insert);
    // now set as marked
    $rated = sprintf("UPDATE wt_LinkedStep4datasets SET rated=1 "
      . "WHERE s4jNo='%s' AND exptId='%s' AND jNo='%s'",
      $s4jNo, $exptId, $content[7]);
    $igrtSqli->query($rated);
  }
//    contentArray[1] = 'haveResponse';
//    contentArray[2] = choice;
//    contentArray[3] = intervalId;
...
//    contentArray[8] = exptId;
  ...
  private function getWholeTranscript($s4jNo) {
    global $igrtSqli;
    $getS4Qry = sprintf("SELECT * FROM wt_LinkedStep4datasets WHERE s4jNo='%s' AND rated=0 ORDER BY jNo ASC", $s4jNo);
    $getS4Result = $igrtSqli->query($getS4Qry);
    if ($getS4Result) {
      $shuffleRow = $getS4Result->fetch_object();
      $dayNo = $shuffleRow->dayNo;
      $sessionNo = $shuffleRow->sessionNo;
      $jNo = $shuffleRow->jNo;
      $exptId = $shuffleRow->exptId;
      $tValues = $this->getTranscriptValues($exptId, $jNo);
      $pretenderRight = mt_rand(0,1);
  ...
  private function getTranscriptValues($exptId, $jNo) {
    global $igrtSqli;    
  ...
  private function assembleTranscript($tValues, $pretenderRight) {
    ...
    for ($i=0; $i<($turnNo-1); $i++) {  // avoid 'FINAL' entry
      $q = $tValues['qArray'][$i];
      $pr = $tValues['pArray'][$i];
      $npr = $tValues['npArray'][$i];
