...
class iStep2BalancerController {
  private $igrtSqli;
  private $htmlBuilder;
  private $tabIndex;
  ...
  function setupBalancer($exptId, $jType, $respNo) {
    ...
    while ($dayRow = $dayResult->fetch_object()) {
      ...
      while ($sessionRow = $sessionResult->fetch_object()) {
        ...
        if ($this->igrtSqli->affected_rows > 0) {
          ...
          $jDiscard =$jType == 0 ? $discardRow->evenDiscards : $discardRow->oddDiscards;
        }
        else {
          $jDiscard = 0;
        }
    ...
    if ($this->igrtSqli->affected_rows > 0) {
      // update
      if ($jType == 0) {
        $updateQry = sprintf("UPDATE wt_Step2BalancerRespMax SET invertedEvenRespMax='%s' WHERE exptId='%s'", $respNo, $exptId);
      }
      else {
        $updateQry = sprintf("UPDATE wt_Step2BalancerRespMax SET invertedOddRespMax='%s' WHERE exptId='%s'", $respNo, $exptId);            
      }
      $this->igrtSqli->query($updateQry);
    }
    else {
      //insert
      if ($jType == 0) {
        $insertQry = sprintf("INSERT INTO wt_Step2BalancerRespMax (exptId, invertedEvenRespMax) VALUES('%s', '%s')", $exptId, $respNo);
      }
      else {
        $insertQry = sprintf("INSERT INTO wt_Step2BalancerRespMax (exptId, invertedOddRespMax) VALUES('%s', '%s')", $exptId, $respNo);
      }
  ...
  function buildHtmlFromSessionList($s2Dataset, $sectionTitle, $sessionPara, $isActiveSet) {
    ...
    $html.= "<div><table><tr><th>title</th><th>max # Odd P</th><th>Odd Action</th><th>max # Even P</th><th>Even Action</th></tr>";
    $evenCounts = array();
    $oddCounts = array();
    ...
    if ($this->igrtSqli->affected_rows > 0) {
      ...
      $oddRespMax = $s2RespRow->invertedOddRespMax;
      $evenRespMax = $s2RespRow->invertedEvenRespMax;
    }
    else {
      $oddRespMax = 20;
      $evenRespMax = 20;        
      ...
      if ($isActiveSet == 0) {
        ...
        $html.= "<br/>$oddS1Label acting as NP to $oddS1Label";
      }
      else {
        $html.= "<b>started: $oddS1Label acting as NP to $oddS1Label</b>";
      }
      $html.= "</td>";
      ...
      if ($isActiveSet == 0) {
        ...
        $html.= "<br />".$this->htmlBuilder->makeButton($buttonId, $buttonLegend, "button"); 
        $html.= "<br/>$evenS1Label acting as NP to $evenS1Label";
      }
      else {
        $html.= "<b>started: $evenS1Label acting as NP to $evenS1Label</b>";
      }
      $html.= "</td>";
      $html.= "</tr>";
    }
  ...
  function getExptDataList() {
    ...
    // find SET B: one step1 marked, one configured, at least one started
    $s2StartedArray = array();
    $s2StatusQry = "SELECT DISTINCT(t1.exptId) AS exptId FROM edSessions AS t1 JOIN wt_Step2BalancerInverted AS t2 "
    ...
    foreach ($s2ConfiguredArray as $es) {
      ...
      if ($isStarted == false) {
        $e_qry = sprintf("SELECT * FROM wt_Step2BalancerInverted WHERE exptId='%s' AND jType=0", $es['exptId']);
        $this->igrtSqli->query($e_qry);
        $hasEven = ($this->igrtSqli->affected_rows > 0) ? 1 : 0;
        $o_qry = sprintf("SELECT * FROM wt_Step2BalancerInverted WHERE exptId='%s' AND jType=1", $es['exptId']);
