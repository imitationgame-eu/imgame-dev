<?php
// -----------------------------------------------------------------------------
// web service to export all inverted Step2 respondents to each Step1 QS for use in nVivo
// -----------------------------------------------------------------------------
...
if ($permissions>=128) {
  ...
  $qsList = array();
  $qsQry = sprintf("SELECT * FROM wt_Step2BalancerInverted WHERE exptId='%s' AND jType='%s' ORDER BY actualJNo ASC", $exptId, $jType);
    ...
    while ($qsRow = $qsResult->fetch_object()) {
      ...
      if ($iQuestionResult) {
        while ($iQuestionRow = $iQuestionResult->fetch_object()) {
          ...
          $s2Qry = sprintf("SELECT * FROM dataSTEP2inverted WHERE exptId='%s'AND jType='%s' AND dayNo='%s' AND "
