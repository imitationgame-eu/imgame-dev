<?php
// -----------------------------------------------------------------------------
// web service to list inverted STEP2 datasets that are ready for download to nVivo
// -----------------------------------------------------------------------------
...
if ($permissions>=128) {
  ...
  if ($uidResult) {
    $html = "<table><tr><th>expt #</th><th>title</th><th># of ppts</th><th>Odd</th><th>Even</th></tr>";
    while ($uidRow = $uidResult->fetch_object()) {
      $exptId = $uidRow->exptId;
      $eModel = new experimentModel($exptId);
      $title = $eModel->title;
      $uniqueEvenPPTQry = "SELECT * FROM wt_Step2BalancerInverted WHERE exptId=$exptId AND jType=0 ORDER BY actualJNo ASC";
      $upptEvenResult = $igrtSqli->query($uniqueEvenPPTQry);
      $pptEvenCnt = $upptEvenResult->num_rows;
      if ($pptEvenCnt > 0) {
        $eRow = $upptEvenResult->fetch_object();        
        // check for data
        $dataQry = sprintf("SELECT * FROM dataSTEP2inverted WHERE "
            . "exptId='%s' AND jType='0' AND dayNo='%s' AND sessionNo='%s'",
            $exptId, $eRow->dayNo, $eRow->sessionNo);
        $hr = $igrtSqli->query($dataQry);
        if ($hr->num_rows == 0) { $eStatusTxt = '<p>no responses</p>'; } else { $eStatusTxt = ''; }
      }
      $uniqueOddPPTQry = "SELECT * FROM wt_Step2BalancerInverted WHERE exptId=$exptId AND jType=1 ORDER BY actualJNo ASC";
      ...
      if ($pptOddCnt > 0) {
        ...
        $dataQry = sprintf("SELECT * FROM dataSTEP2inverted WHERE "
            ...
            $exptId, $eRow->dayNo, $eRow->sessionNo);
        $hr = $igrtSqli->query($dataQry);
        if ($hr->num_rows == 0) { $oStatusTxt = '<p>no responses</p>'; } else { $oStatusTxt = ''; }
      }
