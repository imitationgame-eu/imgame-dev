<?php
// -----------------------------------------------------------------------------
// web service to expose JSON encoded inverted STEP 2 data
// 
// for use by Knockout-JS inverted-step2 reviewer
// 
// -----------------------------------------------------------------------------
...
$exptId = $_GET['exptId'];
$jType = $_GET['jType'];
$reviewType = isset($_GET['reviewType']) ? $_GET['reviewType'] : "";  // not used now - used to be part of button ID
...
//    return $count;
  ...
  function storeContiguousReview($exptId, $jType, $dsList) {
    global $igrtSqli;
    $clearSummariesSql = sprintf("DELETE FROM wt_Step2summariesInverted WHERE exptId='%s' AND jType='%s'", $exptId, $jType);
    $igrtSqli->query($clearSummariesSql);
    $clearReviewsSql = sprintf("DELETE FROM wt_Step2pptReviewsInverted WHERE exptId='%s' AND jType='%s'", $exptId, $jType);
    $igrtSqli->query($clearReviewsSql);
    $clearMarkedDataSql = sprintf("DELETE FROM md_invertedStep2reviewed WHERE exptId='%s' AND jType='%s'", $exptId, $jType);
    ...
    foreach ($dsList as $dsi) {
      ...
      foreach ($dsi['ppts'] as $ppt) {
        ...
        $updatePptReviews = sprintf("INSERT INTO wt_Step2pptReviewsInverted "
            . "(exptId, jType, actualJNo, jNo, dayNo, sessionNo, reviewedRespNo, respNo, ignorePpt, reviewed, finished) "
            . "VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
            $exptId, $jType, $actualJNo, $jNo, $dayNo, $sessionNo, $pptCnt, $respNo, $ignorePpt, $reviewed, $finished);
      ...
      $insertSummarySql = sprintf("INSERT INTO wt_Step2summariesInverted (exptId, jType, dayNo, sessionNo, jNo, pptCnt, actualJNo) "
  ...
  function getUnreviewedData($exptId, $jType) {
    ...
    $dsQry = sprintf("SELECT * FROM wt_Step2BalancerInverted WHERE exptId='%s' AND jType='%s' ORDER BY actualJNo ASC",
        $exptId, $jType);
    ...
    while ($dsRow = $dsResult->fetch_object()) {
      ...
      $pptQry = sprintf("SELECT DISTINCT(pptNo) AS pptNo FROM dataSTEP2inverted WHERE exptId='%s' "
          . "AND jType='%s' AND dayNo='%s' AND sessionNo='%s' AND jNo='%s' ORDER BY uid ASC",
      ...
      if ($pptListResult) {
        while ($pptListRow = $pptListResult->fetch_object()) {
          ...
          $respondentStatusQry = sprintf("SELECT * FROM wt_Step2pptStatusInverted "
              . "WHERE "
              . "exptId='%s' AND jType='%s' AND actualJNo='%s' AND respNo='%s' AND finished='1'",
            $exptId, $jType, $actualJNo, $pptNo);
          $respondentResult = $igrtSqli->query($respondentStatusQry);
          //echo $respondentStatusQry;
          if ($respondentResult) {
            ...
            while ($qRow = $qResult->fetch_object()) {
              ...
              if ($canUse == 1) {
                $question = $qRow->q;
                $qNo = $qRow->qNo;  
                $replyQry = sprintf("SELECT * FROM dataSTEP2inverted WHERE exptId='%s' AND jType='%s' "
                    . "AND dayNo='%s' AND sessionNo='%s' AND jNo='%s' AND uid='%s' AND qNo='%s'",
                    $exptId, $jType, $dayNo, $sessionNo, $jNo, $uid, $qNo);
  ...
  function getReviewedData($exptId, $jType) {
   global $igrtSqli;
    ...
    while ($dsRow = $dsResult->fetch_object()) {
      ...
      if (isset($pptList)) { unset($pptList); }
      $pptList = array();
      // don't need to check for equal qCnt, as this is done when gettting unreviewed data only
      // now make array of complete and incomplete pptNo mappings
      $reviewedRespNo = 0;
      $pptQry = sprintf("SELECT DISTINCT(reviewedRespNo) AS reviewedRespNo FROM md_invertedStep2reviewed WHERE exptId='%s' AND jType='%s' AND actualJNo='%s'",
          $exptId, $jType, $actualJNo);
      $pptListResult = $igrtSqli->query($pptQry);
      while ($pptListRow = $pptListResult->fetch_object()) {
        $reviewedRespNo = $pptListRow->reviewedRespNo;
        $respNoQry = sprintf("SELECT respNo,uid,restartUID FROM md_invertedStep2reviewed WHERE exptId='%s' "
        ...
        // get reviewed/ignore status from wt_Step2pptReviews
        $ignoreQry = sprintf("SELECT * FROM wt_Step2pptReviewsInverted WHERE exptId='%s' AND jType='%s' AND actualJNo='%s' AND uid='%s'", 
            $exptId, $jType, $actualJNo, $uid);
        ...
        $pptStatusQry = sprintf("SELECT * FROM wt_Step2pptStatusInverted WHERE exptId='%s' AND jType='%s' AND actualJNo='%s' AND respNo='%s'",
        ...
        if (isset($turns)) { unset($turns); }
        $turns = array();
        $turnQry = sprintf("SELECT * FROM md_invertedStep2reviewed WHERE exptId='%s' AND jType='%s' AND actualJNo='%s' AND uid='%s' ORDER BY qNo ASC",
          $exptId, $jType, $actualJNo, $uid);
        //echo $turnQry;
      ...
      $pptStatusQry = sprintf("SELECT * FROM wt_Step2pptStatusInverted WHERE exptId='%s' AND jType='%s' AND actualJNo='%s' AND finished='1' ORDER BY respNo ASC",
          $exptId, $jType, $actualJNo);
      $pptStatusResult = $igrtSqli->query($pptStatusQry);
      if ($pptStatusResult) {
  ...
  function processNewRespNo($pptDef, $exptId, $jType, $dayNo, $sessionNo, $jNo, &$wordCnt) {
    ...
    $wordCnt = 0;
    $pptNo = $pptDef['respNo'];
    $getStep1Qry = sprintf("SELECT * FROM md_invertedStep1reviewed WHERE exptId='%s' AND jType='%s' AND "
        . "dayNo='%s' AND sessionNo='%s' AND jNo='%s' AND canUse=1 ORDER by qNo ASC",
        $exptId, $jType, $dayNo, $sessionNo, $jNo);
    ...
    if ($getStep1Result) {
      while ($getStep1Row = $getStep1Result->fetch_object()) {
        ...
        // get reply
        $getReplyQry = sprintf("SELECT * FROM dataSTEP2inverted WHERE exptId='%s' AND jType='%s' AND dayNo='%s'"
...
if ($permissions >= 128) {
  $hasMarkedData = false;
  $qry = "SELECT * FROM md_invertedStep2reviewed WHERE exptId=$exptId AND jType=$jType";
  $mdResult = $igrtSqli->query($qry);

	$hasMarkedData = $mdResult ? ($mdResult->num_rows > 0 ? true : false) : false;
  if (!$hasMarkedData) {
    // create and store first review
    $udsList = getUnreviewedData($exptId, $jType);
    storeContiguousReview($exptId, $jType, $udsList);
    // now re-get to use contiguous respNo, which is important for step3 shuffle
    if (isset($dsList)) { unset($dsList); }
    $dsList = getReviewedData($exptId, $jType);
  }
  else {
    $dsList = getReviewedData($exptId, $jType);
    storeContiguousReview($exptId, $jType, $dsList); // not strictly a first review, but ensure any new unmarked data is correctly represented in wt_Step2pptReview
  }
    ...
    $jSonRep.="\"datasets\":[";
      ...
      foreach ($jsonArray as $ds) {   //DO datasets
        ...
        foreach ($ds['ppts'] as $umi) {   //DO ppts 
          if ($jCnt++ > 0) { $jSonRep.=","; }
          $jSonRep.=sprintf("{\"wordCnt\":%s, \"reviewedRespNo\":%s, \"respNo\":%s, \"jNoLabel\":%s, \"reviewed\":1, \"finished\":\"%s\", \"discardPpt\":\"%s\",", $umi['wordCnt'], 
              $umi['reviewedRespNo'] == '' ? -1 : $umi['reviewedRespNo'], 
              $umi['respNo'] == '' ? -1 : $umi['respNo'], 
              $actualJNo, ($umi['pptFinished'] == 1) ? "finished" : "not finished", ($umi['ignorePpt'] == 1) ? "True" : "" ); 
          $jSonRep.= "\"uid\":".$umi['uid'].",";
          $jSonRep.= "\"restartUID\":".$umi['restartUID'].",";
          if ($umi['restartUID'] == 0) {
            $jSonRep.= "\"pptLabel\":". JSONparse("s2_".$exptId."_".$jType."_".$actualJNo."_".$umi['uid'] ). ",";            
          }
          else {
            $jSonRep.= "\"pptLabel\":". JSONparse("s2_".$exptId."_".$jType."_".$actualJNo."_".$umi['restartUID'] ). ",";            
          }
