...
// -----------------------------------------------------------------------------
// 
// web service to list detail of STEP2 balancer datasets that are in progress
//  
// -----------------------------------------------------------------------------
...
if ($permissions>=128) {
  ...
  $operationLabel = $jType==0 ? "$oddS1Label pretending to be $evenS1Label" : "$evenS1Label pretending as $oddS1Label";
  $qListQry = sprintf("SELECT * FROM wt_Step2Balancer WHERE exptId='%s' AND jType='%s' ORDER BY actualJNo ASC", $exptId, $jType);
  $qResult = $igrtSqli->query($qListQry);
  $html = "no Step2 respondents started";
  if ($qResult->num_rows > 0) {
    $html = "<h2>S2 datasets: $operationLabel</h2>";
