...
// -----------------------------------------------------------------------------
// 
// web service to support AJAX calls from Step2Respondent Monitor
...
$htmlBuilder = new htmlBuilder();
  ...
  function getTranscript($userCode) {
    ...
    $restartUID = $params[4]; //
    $pptUid = $params[5];
    $statusQry = sprintf("SELECT * FROM wt_Step2pptStatus WHERE id='%s'", $pptUid);
    $debug = $statusQry;
    $statusResult = $igrtSqli->query($statusQry);
    $statusRow = $statusResult->fetch_object();
    $respNo = $statusRow->respNo;
    $actualJNo = $statusRow->actualJNo;
    $balQry = sprintf("SELECT * FROM wt_Step2Balancer WHERE exptId='%s' AND jType='%s' AND actualJNo='%s'",
...
//    // get link between pptUid and the actual uid used in dataSTEP2
//    $uidLinkQry = sprintf("SELECT * FROM wt_Step2pptStatus wHERE exptId='%s' AND jType='%s' AND actualJNo='%s' AND restartUID='%s'",
//        $exptId, $jType, $actualJNo, $pptUid);
//    $uidResult = $igrtSqli->query($uidLinkQry);
//    $uidRow = $uidResult->fetch_object();
    $s2uid = $pptUid;
    $getQ = sprintf("SELECT * FROM md_dataStep1reviewed WHERE exptId='%s' AND jType='%s' AND dayNo='%s' "
        . "AND sessionNo='%s' AND jNo='%s' AND canUse=1 AND q != 'FINAL' ORDER BY qNo ASC",
    ...
    while ($qRow = $qResult->fetch_object()) {
      ...
      $getSingleResponseQry = sprintf("SELECT * FROM dataSTEP2 WHERE exptId='%s' AND jType='%s' "
          . "AND dayNo='%s' AND sessionNo='%s' AND jNo='%s' AND uid='%s'"
          . "AND qNo='%s'",
          $exptId, $jType, $dayNo, $sessionNo, $jNo, $s2uid, $qRow->qNo);
  ...
  function getJudgeDataSet($exptId, $jType, $actualJNo) {
    ...
    global $htmlBuilder;
    $tintNo = 0;
    $doneRespQry = sprintf("SELECT * FROM wt_Step2pptStatus WHERE exptId='%s' AND jType='%s' AND "
        . "actualJNo='%s' AND finished=1 ORDER BY chrono ASC",
        $exptId, $jType, $actualJNo);
    $doneRespResult = $igrtSqli->query($doneRespQry);
    $doneCnt = $doneRespResult->num_rows;
    $startedRespQry = sprintf("SELECT * FROM wt_Step2pptStatus WHERE exptId='%s' AND jType='%s' AND "
    ...
    $discardedRespQry = sprintf("SELECT * FROM wt_Step2pptStatus WHERE exptId='%s' AND jType='%s' AND "
  ...
  function processDiscardPpt($exptId, $jType, $userCode) {
    ...
    $updateQry = sprintf("UPDATE wt_Step2pptStatus SET discarded=1 WHERE userCode='%s'", $userCode);
    $igrtSqli->query($updateQry);
    $label = "s2_".$exptId."_".$jType."_ds".$dsStr;
    $updateBalQry = sprintf("UPDATE wt_Step2Balancer SET respMax=respMax+1 WHERE label='%s'", $label);
