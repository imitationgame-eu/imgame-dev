...
if (!isset($_POST['exptId'])) { die('no exptId given'); }
$exptId=$_POST['exptId'];
$dayNo=$_POST['dayNo'];
...
for ($dayPtr=$dayNo; $dayPtr<=$dayNo+1; $dayPtr++) {
	$sessions = [];
	for ($sessionPtr=$sessionNo; $sessionPtr<=$$sessionNo+1; $sessionPtr++) {
		$owners = [];
...
$fileName = "classicData_".$exptId."_d".$dayNo."_s".$sessionNo.".csv";
