...
require_once($root_path.'/helpers/step4/class.neStep4Controller.php');
$s4jNo = $_GET['s4jNo'];
...
//
  function processMessage($_messageType, $_content, $s4jNo, $exptId, $jType, $actualJNo, $respNo, $qNo) {
    ...
    switch ($_messageType) {
      ...
      case "nextPage": {
        $s4params = $s4Controller->neGetStep4CurrentStatus($exptId, $jType, $s4jNo);
        if ($s4params['status'] == 'done') {
          $usePost = $s4Controller->getUsePost($exptId) == 1 ? "usePost" : "noPost";
          $msg = sprintf("<message><messageType>s4complete1</messageType><usePost>%s</usePost></message>", $usePost);
        }
        else {
          $msg = sprintf("<message>"
            ...
            . "<respNo1>%s</respNo1>"
            . "<respNo2>%s</respNo2>"
            . "<s3respNo1>%s</s3respNo1>"
            . "<s3respNo2>%s</s3respNo2>"
            . "<qNo>%s</qNo>"
            . "<form><![CDATA[%s]]></form>"
            . "</message>", 
