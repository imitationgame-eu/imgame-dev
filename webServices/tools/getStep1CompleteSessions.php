      ...
      if ( ($discardType & $discardMarker) != $discardMarker) {
        ...
        if ($gameSetResults) {
          ...
          while ($gameSetRow = $gameSetResults->fetch_object()) {
            $sqlGetData=sprintf("SELECT * FROM md_dataStep1reviewed WHERE exptId='%s' AND jType='%s' AND jNo='%s' AND dayNo='%s' AND sessionNo='%s' AND qNo='%s'",
                              $exptId, $jType, $jNo, $dayNo, $sessionNo, $qNo); 
            $jResult = $igrtSqli->query($sqlGetData);
            $discardQRow = $jResult->fetch_object();
            ++$qNo;
            if ($discardQRow->canUse == 1) {            
              if ($cb == 0) {
              ...
              if ($gameSetRow->qNo == $finalQNo) {
                //$reason = rawurlencode
                echo "Final Rating: $gameSetRow->q <br />";
