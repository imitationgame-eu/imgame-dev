    ...
    foreach ($datalines as $dl) {
      if ($dl["count"] > 0) {
        ...
        if ($msgType == "<JQ>") {
          ...
          $playerNo = $details[1];
          $turnPtr = $details[2];
          $iIntention = urldecode($details[4]);
          ...
          $judgeArray[$playerNo]['turns'][$turnPtr]['iIntention'] = $iIntention;
          $judgeArray[$playerNo]['turns'][$turnPtr]['r1Alignment'] = '';
          $judgeArray[$playerNo]['turns'][$turnPtr]['r2Alignment'] = '';
          $judgeArray[$playerNo]['turns'][$turnPtr]['categoryAlignment'] = '';
        }
      }
    }
  }
...
for ($i=0; $i<$dlCnt; $i++) {
  ...
  if ($datalines[$i]["count"] > 0) {
    ...
    switch ($msgType) {
      ...
      case '<JR>' : {
        // <JR><:>9<:>1<:>1<:>arvostan%20hyvin%20!<:>interval4<:>0<:>respondent1Interval4<:>respondent2Interval4<:>categoryInterval3        
        ...
        $npSide = $details[6];
        $r1Alignment = $details[7];
        $r2Alignment = $details[8];
        $categoryAlignment = $details[9];
        ...
        $judgeArray[$playerNo]['turns'][$turnPtr]['r1Alignment'] = $r1Alignment;
        $judgeArray[$playerNo]['turns'][$turnPtr]['r2Alignment'] = $r2Alignment;
        $judgeArray[$playerNo]['turns'][$turnPtr]['categoryAlignment'] = $categoryAlignment;
        break;
      }
      case '<JfinalR>' : {
        $playerNo = $details[1];  // NB - this is the J game# 
...
$delSql = "DELETE FROM dataSTEP1 WHERE exptId=306";
  ...
  if ($qCount > 0) {
    for ($j=0; $j<$qCount; $j++) {
      ...
      if ($judgeArray[$i]['turns'][$j]['isFinal']) {
            ...
            $sql = sprintf("INSERT INTO dataSTEP1 "
                . "(uid, exptId, jType, jNo, sessionNo, dayNo, npLeft, qNo, q, npr, pr, choice, rating, reason, iIntention, r1Alignment, r2Alignment, categoryAlignment) "
                ...
                $judgeArray[$i]['turns'][$j]['choice'],         
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['confidence']),         
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['reason']),
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['iIntention']),         
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['r1Alignment']),         
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['r2Alignment']),         
                $igrtSqli->real_escape_string($judgeArray[$i]['turns'][$j]['categoryAlignment'])                
            );        
      }
      echo $sql."<br />";
      $igrtSqli->query($sql);
