<?php
// stratification of neighbourhood racial mix 
// 1 = very mixed
// 2 = a little mixed
// 3 = not mixed
// [1+2 combined]
  ...
  function getS2attributes($actualJNo) {
    ...
    if ($igrtSqli->affected_rows > 0) {
      while ($eRow = $eResult->fetch_object()) {
        if ($eRow->neighbourhoodRacialMix == 3) {
    ...
    if ($igrtSqli->affected_rows > 0) {
      while ($oRow = $oResult->fetch_object()) {
        if ($oRow->neighbourhoodRacialMix == 3) {
  ...
  function getS4attributes($actualJNo) {
    ...
    if ($igrtSqli->affected_rows > 0) {
      while ($eRow = $eResult->fetch_object()) {
        ...
        if ($igrtSqli->affected_rows > 0) {
          $aRow = $aResult->fetch_object();
          $a = $aRow->neighbourhoodRacialMix == 3 ? 0 : 1;
          if ($a == 1) {
...
if ($igrtSqli->affected_rows > 0) {
  ...
  echo 'for columns A to E,1=not mixed,2=mixed<br/>';
