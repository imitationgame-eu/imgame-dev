...
include_once $root_path.'/domainSpecific/mySqlObject.php';       
  ...
  function getJType($uid) {
    return ($uid % 2 == 0) ? 0 : 1;
  }
  
  function getUid($text) {
    global $igrtSqli;
    $getUidQry = sprintf("SELECT id FROM igUsers WHERE email='%s'", $text);
  ...
  function getJDetails($text) {
    $uid = getUid($text);
    $jType = getJType($uid);
    return array('uid'=>$uid, 'jType'=>$jType);
  }
  ...
  function reOrder($dataItems) {
    usort($dataItems, function($a, $b) { return $a['uid'] - $b['uid']; });
    return $dataItems;
  }
...
for ($i=0; $i<count($datalines); $i++) {
  ...
  $jDetails = getJDetails($components[10]);
//  echo count($components).' items. '.$components[10].' '.print_r($jDetails,true).'<br />';
//  showComponents($components);
  $dataItem = array(
    ...
    'Afrikaans' => $components[31] > '' ? 1 : 0,
    'English' => $components[32] > '' ? 1 : 0,
    ...
    'friendType5' => $components[66],
    'friendType6' => $components[67]
  );
  array_push($dataItems, $dataItem);
}
$sortedDataItems = reOrder($dataItems);

$s1stratTblName = "xs_S1_".$exptId;
$tblExistsQry = sprintf("show tables like '%s'", $s1stratTblName);
...
if ($igrtSqli->affected_rows == 0) {
  $createTblQry = sprintf("CREATE TABLE `%s` (
    ...
    PRIMARY KEY (`id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8", $s1stratTblName);
  $igrtSqli->query($createTblQry);
  echo $createTblQry.'<br />';
}


for($i=0; $i<count($sortedDataItems); $i++) {
  $dataItem = $sortedDataItems[$i];
  $dataItem['jNo'] = round($i / 2, 0, PHP_ROUND_HALF_DOWN);
  echo $dataItem['jType'].' '.$dataItem['uid'].' '.$dataItem['jNo'].'<br />';
  $insertQry = sprintf("INSERT INTO %s (date,jType,jNo,step1Id,effort,"
